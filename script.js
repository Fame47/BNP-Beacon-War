/* =========================================================
   BEACON WARS v192 CLEAN LAUNCH CORE
   =========================================================
...
function aiCanSafelyBeatKnown(attacker,target){
  ...
  // Objectives / special units follow the real combat rules.
  if(target.beacon) return true;
  if(target.mine) return !!attacker.engineer;

  // Infiltrator wins only when it attacks a Commander.
  if(attacker.infiltrator) return target.rank===10;

  // Any normal attacker catches a defending Infiltrator.
  if(target.infiltrator) return true;

  // Known equal/lower-rank attacks are deliberate suicides/trades, so do not take them.
  return aiPieceRank(attacker)>aiPieceRank(target);
}

function aiTargetPriority(target){
  if(!target) return 0;
  if(target.beacon) return 10000;
  if(target.rank===10) return 9000;
  if(target.infiltrator) return 4000;
  return 1000 + aiPieceRank(target)*50;
}

function aiDistanceToCell(piece,r,c){
  return Math.abs(piece.r-r)+Math.abs(piece.c-c);
}

function aiBestEmptyStepToward(unit,target,stopAdjacent=false){
  if(!unit||!target) return null;
  const options=getLegal(unit).filter(t=>!board[t.r][t.c]);
  ...
  options.sort((a,b)=>{
    ...
    return nearestPlayerGoalScore(b.r,b.c)-nearestPlayerGoalScore(a.r,a.c);
  });
...
function aiTurn(){
  ...
  for(let r=0;r<ROWS;r++)for(let c=0;c<COLS;c++){
    const p=board[r][c];
    if(!p) continue;
    if(p.team===enemyTeam()&&p.movable) units.push(p);
    else if(p.team===playerTeam()) players.push(p);
  }
  ...
  const hiddenPlayers=players.filter(p=>!aiKnows(p));
  const revealedPlayers=players.filter(p=>aiKnows(p));
  let pick=null;

  /* INFORMATION RULE 1:
     If a player piece is still unknown, probe it with the LOWEST-ranked
     available enemy first. If the low piece can attack now, attack to reveal. */
  if(hiddenPlayers.length){
    const probeAttacks=[];
    units.forEach(u=>{
      getLegal(u).forEach(t=>{
        const target=board[t.r][t.c];
        if(target&&target.team===playerTeam()&&!aiKnows(target)){
          probeAttacks.push({type:'attack',u,t,target});
        }
      });
    });

    if(probeAttacks.length){
      probeAttacks.sort((a,b)=>{
        const rankDiff=aiPieceRank(a.u)-aiPieceRank(b.u);
        if(rankDiff!==0) return rankDiff;
        return aiTargetPriority(b.target)-aiTargetPriority(a.target);
      });
      pick=probeAttacks[0];
    }

    // No immediate probe attack. Send the lowest-ranked movable enemy toward
    // the nearest unknown player.
    if(!pick){
      const probes=units.slice().sort((a,b)=>aiPieceRank(a)-aiPieceRank(b));
      for(const u of probes){
        const target=hiddenPlayers.slice().sort(
          (a,b)=>aiDistanceToCell(u,a.r,a.c)-aiDistanceToCell(u,b.r,b.c)
        )[0];
        const step=aiBestEmptyStepToward(u,target,false);
        if(step){
          pick={type:'move',u,t:step,target,mode:'probe'};
          break;
        }
      }
    }
  ...
  /* INFORMATION RULE 2:
     Once a player is revealed, send the HIGHEST-ranked enemy that can
     safely beat that known target. Never knowingly suicide a lower/equal rank. */
  if(!pick && revealedPlayers.length){
    const safeAttacks=[];
    units.forEach(u=>{
      getLegal(u).forEach(t=>{
        const target=board[t.r][t.c];
        if(target&&target.team===playerTeam()&&aiKnows(target)&&aiCanSafelyBeatKnown(u,target)){
          safeAttacks.push({type:'attack',u,t,target});
        }
      });
    });

    if(safeAttacks.length){
      safeAttacks.sort((a,b)=>{
        const rankDiff=aiPieceRank(b.u)-aiPieceRank(a.u);
        if(rankDiff!==0) return rankDiff; // highest safe attacker first
        return aiTargetPriority(b.target)-aiTargetPriority(a.target);
      });
      pick=safeAttacks[0];
    }

    // If no immediate safe attack exists, send the highest safe hunter toward
    // a revealed target.
    if(!pick){
      const huntPlans=[];
      revealedPlayers.forEach(target=>{
        units.filter(u=>aiCanSafelyBeatKnown(u,target)).forEach(u=>{
          const step=aiBestEmptyStepToward(u,target,false);
          if(step){
            huntPlans.push({
              type:'move',u,t:step,target,mode:'hunt',
              rank:aiPieceRank(u),
              distance:aiDistanceToCell(u,target.r,target.c)
            });
          }
        });
      });
      ...
      if(huntPlans.length){
        huntPlans.sort((a,b)=>{
          ...
          if(aiTargetPriority(a.target)!==aiTargetPriority(b.target)){
            return aiTargetPriority(b.target)-aiTargetPriority(a.target);
          }
    ...
    // Nobody can safely beat a revealed target. Bluff with the highest-ranked
    // available unit: move close/adjacent, but never enter the target square.
    if(!pick){
      const bluffers=units.slice().sort((a,b)=>aiPieceRank(b)-aiPieceRank(a));
      for(const u of bluffers){
        const target=revealedPlayers.slice().sort(
          (a,b)=>aiDistanceToCell(u,a.r,a.c)-aiDistanceToCell(u,b.r,b.c)
        )[0];
        const step=aiBestEmptyStepToward(u,target,true);
        if(step){
          pick={type:'move',u,t:step,target,mode:'bluff'};
          break;
        }
      }
    }
  ...
  // Fallback patrol if no information-driven plan can move.
  if(!pick){
    const patrol=[];
    units.forEach(u=>{
      getLegal(u).forEach(t=>{
        if(!board[t.r][t.c]){
          patrol.push({type:'move',u,t,score:nearestPlayerGoalScore(t.r,t.c)});
        }
      });
    });
    patrol.sort((a,b)=>b.score-a.score);
    pick=patrol[0]||null;
  }
  ...
  if(pick){
    if(pick.type==='attack'){
      ...
      resolveCombat(pick.u,board[pick.t.r][pick.t.c]);
    }else{
      playSound('movePiece',{volume:.62});
      ...
      if(pick.mode==='bluff') log(teamLabel(enemyTeam())+' unit moved into bluffing range.');
      else if(pick.mode==='probe') log(teamLabel(enemyTeam())+' unit advanced to probe an unidentified contact.');
      else if(pick.mode==='hunt') log(teamLabel(enemyTeam())+' unit advanced on a revealed target.');
      else log(teamLabel(enemyTeam())+' unit advanced.');
    }
  }
